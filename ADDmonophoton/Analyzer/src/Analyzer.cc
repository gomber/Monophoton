  ...
  if(!isAOD_){ 
   ...
   if(runphotons_){
	 ...
	 //std::cout<<"Photon pt/eta/phi:" << photon->et()<<"\t"<<photon->eta() <<"\t"<< photon->phi()<<  std::endl;
       ...
       std::cout<<"coming here I:"<<std::endl;
     }
...
// Rho correction with max eta 2.5
 ...
 if(sigmaHandle25.isValid()) {      
   ...
   if(runelectrons_){
     ...
     std::vector<reco::GsfElectron>  myelectron_container;
     myelectron_container.clear();
     ...
     iEvent.getByLabel("selectedPatElectrons",electronHandle);
     edm::View<pat::Electron>::const_iterator pele;

     
     for(reco::GsfElectronCollection::const_iterator ele_iter = electrons->begin(); ele_iter != electrons->end(); ++ele_iter) {
       myelectron_container.push_back(*ele_iter);
     }


     Electron_n = 0;
     for(pele= electronHandle->begin();pele!=electronHandle->end();++pele){
       for(unsigned int x=0;x <electrons->size();x++){
	 
	 reco::GsfElectronRef ele(electrons, x);
	 if (ele->et() != pele->et()) continue;
	 
	 double iso_ch =  (*(isoVals)[0])[ele];
	 double iso_em = (*(isoVals)[1])[ele];
	 double iso_nh = (*(isoVals)[2])[ele];
	 
	 eleiso_ch[x]=(*(isoVals)[0])[ele];
	 eleiso_em[x]=(*(isoVals)[1])[ele];
	 eleiso_nh[x]=(*(isoVals)[2])[ele];
	 
	 std::cout<<"Electron iso for :"<<x<<"the electron charged:"<<eleiso_ch[x]<<"em: "<<eleiso_em[x]<<"nh: "<<eleiso_nh[x]<<std::endl;
	 // working points                                                                                                                                                                       
	 electron_veto[x]       = EgammaCutBasedEleId::PassWP(EgammaCutBasedEleId::VETO, ele, conversions_h, beamSpot, vtx_h, iso_ch, iso_em, iso_nh, rho);
	 electron_loose[x]      = EgammaCutBasedEleId::PassWP(EgammaCutBasedEleId::LOOSE, ele, conversions_h, beamSpot, vtx_h, iso_ch, iso_em, iso_nh, rho);
	 electron_medium[x]     = EgammaCutBasedEleId::PassWP(EgammaCutBasedEleId::MEDIUM, ele, conversions_h, beamSpot, vtx_h, iso_ch, iso_em, iso_nh, rho);
	 electron_tight[x]      = EgammaCutBasedEleId::PassWP(EgammaCutBasedEleId::TIGHT, ele, conversions_h, beamSpot, vtx_h, iso_ch, iso_em, iso_nh, rho);
	 electron_fbremeopin[x] = EgammaCutBasedEleId::PassEoverPCuts(ele);
	 electron_trigtight[x] = EgammaCutBasedEleId::PassTriggerCuts(EgammaCutBasedEleId::TRIGGERTIGHT, ele);
	 electron_vtxFitConversion[x] =!ConversionTools::hasMatchedConversion(*ele, conversions_h, beamSpot.position());

	 electron_pt[x]       = myelectron_container[x].pt();
	 std::cout<<"Pt from electron container: "<<electron_pt[x]<<std::endl;
	 ...
	 electron_eopin[x]         = myelectron_container[x].eSuperClusterOverP();
	 electron_fbrem[x]         = myelectron_container[x].fbrem();    
	 electron_ooemoop[x]       = ((1.0/ myelectron_container[x].ecalEnergy()) - ( myelectron_container[x].eSuperClusterOverP()/ myelectron_container[x].ecalEnergy()));

	 electron_mHits[x] = myelectron_container[x].gsfTrack()->trackerExpectedHitsInner().numberOfHits(); 

	 if (vtx_h->size() > 0) {
	   reco::VertexRef vtx(vtx_h, 0);    
	   electron_d0vtx[x] = myelectron_container[x].gsfTrack()->dxy(vtx->position());
	   electron_dzvtx[x] = myelectron_container[x].gsfTrack()->dz(vtx->position());
	 } else {
	   electron_d0vtx[x] = myelectron_container[x].gsfTrack()->dxy();
	   electron_dzvtx[x] = myelectron_container[x].gsfTrack()->dz();
	 }
	 std::cout<<"veto: "<<electron_veto[x]<<"loose: "<<electron_loose[x]<<"medium: "<<electron_medium[x]<<"tight: "<<electron_tight[x]<<"fbrem: "<<electron_fbremeopin[x]<<"trigtight: "<<electron_trigtight[x]<<"electron_vtxFitConversion[x] "<<electron_vtxFitConversion[x]<<"electron_eopin[x]: "<<electron_eopin[x]<<" electron_fbrem[x] : "<<electron_fbrem[x]<<"electron_ooemoop[x]: "<<electron_ooemoop[x]<<"electron_mHits[x]: "<<electron_mHits[x]<<"electron_d0vtx[x]: "<<electron_d0vtx[x]<<"electron_dzvtx[x] "<<electron_dzvtx[x]<<std::endl;
	 Electron_n++;
       }//end of for loop
     }
   }
...
if(runPileUp_){      
  ...
  if (runelectrons_){
    ...
    myEvent->Branch("Eleiso_ch",eleiso_ch,"eleiso_ch[Electron_n]/F");
    myEvent->Branch("Eleiso_em",eleiso_ch,"eleiso_em[Electron_n]/F");
    myEvent->Branch("Eleiso_nh",eleiso_nh,"eleiso_nh[Electron_n]/F");
    myEvent->Branch("Electron_veto",electron_veto,"electron_veto[Electron_n]/O");
    myEvent->Branch("Electron_loose",electron_loose,"electron_loose[Electron_n]/O");
    myEvent->Branch("Electron_medium",electron_medium,"electron_medium[Electron_n]/O");
    myEvent->Branch("Electron_tight",electron_tight,"electron_tight[Electron_n]/O");
    myEvent->Branch("Electron_fbremeopin",electron_fbremeopin,"electron_fbremeopin[Electron_n]/O");
    myEvent->Branch("Electron_trigtight",electron_trigtight,"electron_trigtight[Electron_n]/O");
    myEvent->Branch("Electron_vtxFitConversion",electron_vtxFitConversion,"electron_vtxFitConversion[Electron_n]/O");
    myEvent->Branch("Electron_eopin",electron_eopin,"electron_eopin[Electron_n]/F");
    myEvent->Branch("Electron_fbrem",electron_fbrem,"electron_fbrem[Electron_n]/F");
    myEvent->Branch("Electron_ooemoop",electron_ooemoop,"electron_ooemoop[Electron_n]/F");
    myEvent->Branch("Electron_mHits",electron_mHits,"electron_mHits[Electron_n]/I");
    myEvent->Branch("Electron_d0vtx",electron_d0vtx,"electron_d0vtx[Electron_n]/F");
    myEvent->Branch("Electron_dzvtx",electron_dzvtx,"electron_dzvtx[Electron_n]/F");
  }
